...
class ControllerPaymentVeritransbin extends Controller {
  ...
  public function index() {
    ...
    if (file_exists(DIR_TEMPLATE . $this->config->get('config_template')
          . '/template/payment/veritransbin.tpl')) {
      $this->template = $this->config->get('config_template')
        . '/template/payment/veritransbin.tpl';
    } else {
      $this->template = 'default/template/payment/veritransbin.tpl';
    }
  ...
  public function process_order() {
    ...
    try {
      ...
      if ($this->config->get('veritransbin_installment_option') == 'all_product') {
        ...
        if ($this->config->has('veritransbin_installment_banks')) {
          ...
          foreach ($this->config->get('veritransbin_installment_banks')
            ...
            foreach ($this->config->get('veritransbin_installment_' . $key . '_term')
                as $month => $val_month) {
              $terms[] = $month;
            }
      ...
      if (strlen($this->config->get('veritransbin_bin_number')) > 0) {
        // add BIN Filter
  ...
  public function payment_notification() {
    ...
    if ($transaction == 'capture') {
      ...
      if ($fraud == 'challenge') {
        ...
        $this->model_checkout_order->update(
            $notif->order_id,
            $this->config->get('veritransbin_vtweb_success_mapping'),
      ...
      $this->model_checkout_order->update(
          $notif->order_id,
          $this->config->get('veritransbin_vtweb_failure_mapping'),
